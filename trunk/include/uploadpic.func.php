...
function UploadImage($upname,$smallmark=1,$dstsw,$dstsh=0,$path_dim,$path_xim,$newname,$smallname=0) {
    ...
    if (!is_uploaded_file($_FILES[$filename][tmp_name])) {
        if ($refer) {
            ...
            echo "<div class='showmag'><p>上传失败，文件过大或者文件不存在！</p><p><a href='index.php'>返回主页</a></p></div>";
            wapfooter();
            exit;
        }
    }
    ...
    if($max_file_size < $file["size"]) { //检查文件大小
        if ($refer) {
            ...
            echo "<div class='showmag'><p>上传图片过大，请控制在2M之内！</p><p><a href='index.php'>返回主页</a></p></div>";
            wapfooter();
            exit;
        }
    }
    if(!in_array($file["type"],$uptypes)) { //检查文件类型
        if ($refer) {
            ...
            echo "<div class='showmag'><p>上传失败，图片格式不正确！</p><p><a href='index.php'>返回主页</a></p></div>";
            wapfooter();
            exit;
        }
    }
    ...
    if(!move_uploaded_file ($filename,$all_path)) {
        if ($refer) {
            ...
            echo "<div class='showmag'><p>上传失败，附件文件不存在！</p><p><a href='index.php'>返回主页</a></p></div>";
            wapfooter();
            exit;
        }
    }
